...
public class Test {
    ...
    private static Set<Modes4Accessibility> computingModes;
    private static final String accessibilityOutputDirectory = OUTPUTDIR+"accessibilities";
    private static final Double cellSize = 200.;
    private static final int searchInterval = 1;
    static String actType = FacilityTypes.WORK;
	...
	public static void main(String[] args) {
		...
		Test.createControler(CONFIGFILE, ACTIVATE_OTFVIS_OR_NOT, ACTIVATE_PT_OR_NOT, 
	...
	public static Controler createControler(String CONFIGFILE, boolean otfvis_on, boolean pt_on, 
		...
		if (accessibility_on) {
			...
			computingModes = accConfig.getIsComputingMode();
		}
...
// ---------------------simulation loading---------------------------------------			
		...
		*/
					...
					public void install() {
						addControlerListenerBinding().toProvider(new Provider<ControlerListener>() {
							...
							public ControlerListener get() {
								...
								GridBasedAccessibilityShutdownListenerV3 listener = new GridBasedAccessibilityShutdownListenerV3(accessibilityCalculator, activityFacilities, null,
										scenario, bb.getXMin(),bb.getYMin(), bb.getXMax(), bb.getYMax(), cellSize); //activityFacilities!!!!
		...
		controler.run();
			...
			Map<Id<ActivityFacility>, Map<String, Double>> mapOfAccessibilitySumMaps = new HashMap<>();
			Map<Id<ActivityFacility>, Map<String, Double>> mapOfAccessibilityGiniCoefficientMaps = new HashMap<>();
			...
			Test.writePlottingData(OUTPUTDIR, analysisPoints, mapOfAccessibilitySumMaps, "AccessibilitySums.csv");
			Test.writePlottingData(OUTPUTDIR, analysisPoints, mapOfAccessibilityGiniCoefficientMaps, "GiniCoefficients.csv");
		}
	...
	public static void writePlottingData(String outputDirectory, ActivityFacilities analysisPoints, Map<Id<ActivityFacility>, Map<String, Double>> map, String fileName) {
		Log.info("Writing plotting data for other analysis into" + outputDirectory+ "...");
		...
		for (Id<ActivityFacility> analysisPointId : map.keySet()) {
		 ...
		 // for (Modes4Accessibility mode : .....)
			double value = valuesByMode.get(Modes4Accessibility.car.name());
			Gbl.assertNotNull(value);
			writer.writeField(value);
			
			writer.writeNewLine();
			
			double value2 = valuesByMode.get(Modes4Accessibility.freespeed.name());
			Gbl.assertNotNull(value2);
			writer.writeField(value2);
			
			writer.writeNewLine();
		}
		
		writer.close();
		Log.info("Writing Plotting data done!");
	}
}
